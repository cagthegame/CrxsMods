...
${tradutor.texto1[9]} ${user.premiumTime > 0 ? '✅' : (isPrems ? '✅' : '❌') || ''}
 ...
 ${tradutor.texto1[16]}
  ...
  ▢ _${usedPrefix}viewimage_`.trim();
    let pp;
    const menusetpp = global.db.data.users[m.sender].language === 'pt-br'
    if (menusetpp) { 
      pp = imagen7
    } else if (!menusetpp) {
      pp = imagen4
...
handler.command = /^(menuSecreto|menúSecreto|memusecreto|memúsecreto|helpsecreto|infosecreto|comandossecreto|allmenusecreto)$/i;
handler.exp = 50;
handler.fail = null;
